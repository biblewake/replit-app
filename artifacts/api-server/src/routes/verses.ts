...
router.post("/verses/by-reference", async (req, res) => {
  const { reference, version = "NIV" } = req.body as { reference: string; version?: string };
  if (!reference) {
  ...
  try {
    ...
    if (!choice || choice.finish_reason !== "stop") {
      req.log.warn({ finish_reason: choice?.finish_reason }, "verse lookup incomplete");
    ...
    req.log.error(err, "verse lookup failed");
    res.status(500).json({ error: "Failed to fetch verse" });
  }
});
...
router.post("/verses/suggest", async (req, res) => {
  const { theme, version = "NIV" } = req.body as { theme?: string; version?: string };
  try {
    ...
    if (choice?.finish_reason === "stop" && raw) {
      try {
        ...
        if (parsed.reference && parsed.text) {
          res.json({ ...parsed, version: parsed.version ?? version });
          return;
        }
      } catch (_) {}
    }
    req.log.warn({ finish_reason: choice?.finish_reason }, "verse suggest fallback");
    const fallback = FALLBACK_VERSES[Math.floor(Math.random() * FALLBACK_VERSES.length)];
    res.json({ ...fallback, version });
  } catch (err) {
    req.log.error(err, "verse suggest failed");
    const fallback = FALLBACK_VERSES[Math.floor(Math.random() * FALLBACK_VERSES.length)];
    res.json({ ...fallback, version });
  }
});
